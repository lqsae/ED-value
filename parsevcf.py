...
class Record:
    ...
    def sample_info(self, sample, sep=':', mistype='./.'):
        ...
        if sample_tags.startswith(mistype):
            return None
        else:
    ...
    def get_sample_ad(self, sample, ad='AD'):
        ...
        :return: tuple 例如：(1,2)
        """
        SampleAD = collections.namedtuple('dp', 'ref alt')
        sample_info_dict = self.sample_info(sample)
        if sample_info_dict:
            sample_ad = sample_info_dict[ad]
            ref_dp = int(sample_ad.split(',')[0])
            alt_dp = int(sample_ad.split(',')[1])
            # SampleAD._make([ref_dp, alt_dp])
        else:
            ref_dp = 0
            alt_dp = 0
            # SampleAD._make([ref_dp, alt_dp])
...
def main():
    ...
    # pic = args.pic
    fst = args.fst
    pop_list = args.poplist
    if fst:
        info = FST(vcf, pop_list)
        # print(info.samples)
        # print(info.map_index)
        infos= sorted(info, key=lambda x: x.combine)
        for group, record in itertools.groupby(infos, key=lambda x: x.combine):
            # print(record)
            sum_fst = 0
            length = 0
            for info in record:
